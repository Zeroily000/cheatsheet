...
bool BetweenFactor::RightOplusResidual(double const * const * const parameters,
   ...
   *     [         0,                 1] [   0,      1] [    0,     1]
   *
   *   = [r_R_b^{-1}·r_R_a·a_R_b, r_R_b^{-1}·(r_R_a·a_t_b + r_t_a - r_t_b)]
   *     [                     0,                                        1]
   *
   * e_w = Log(r_R_b^{-1}·r_R_a·a_R_b)
   * e_t = r_R_b^{-1}·(r_R_a·a_t_b + r_t_a - r_t_b)
   */
  // clang-format on
  ...
  if (jacobians != nullptr) {
    if (jacobians[0] != nullptr) {
       ...
       *           = Jr^{-1}(e_w)·a_R_b^{-1}
       *
       * d/dwa e_t = d/dwa (r_R_b^{-1}·r_R_a·Exp(wa)·a_t_b
       *           = d/dwa (r_R_b^{-1}·r_R_a·(I + [wa]x)·a_t_b
       *           = d/dwa (r_R_b^{-1}·r_R_a·[wa]x·a_t_b)
       *           = d/dwa -(r_R_b^{-1}·r_R_a·[a_t_b]x·wa)
    ...
    if (jacobians[2] != nullptr) {
       ...
       *           = -Jl^{-1}(e_w)
       *
       * d/dwb e_t = d/dwb (r_R_b·Exp(wb))^{-1}·(r_R_a·a_t_b + ref_t_a - ref_t_b)
       *           = d/dwb Exp(-wb)·r_R_b ^{-1}·(r_R_a·a_t_b + ref_t_a - ref_t_b)
       *           = d/dwb (I - [w]x)·e_t
       *           = d/dwb -[w]x·e_t
       *           = [e_t]x
       */
...
bool BetweenFactor::LeftOplusResidual(double const * const * const parameters,
   ...
   * e = z·r_T_b^{-1}·r_T_a
   *
   *   = [a_R_b, a_t_b]·[r_R_b^{-1}, -r_R_b^{-1}·r_t_b]·[r_R_a, r_t_a]
   *     [    0,     1]   [         0,                   1]   [   0,      1]
   *
   *   = [a_R_b·r_R_b^{-1}·r_R_a, a_R_b·r_R_b^{-1}·(r_t_a - r_t_b) + a_t_b]
   *     [                         0,                                            1]
   *
   * e_w = Log(a_R_b·r_R_b^{-1}·r_R_a)
   * e_t = a_R_b·r_R_b^{-1}·(r_t_a - r_t_b) + a_t_b
   */
  // clang-format on
  Eigen::Quaterniond const b_q_r{r_q_b.inverse()};
  Eigen::Quaterniond const a_q_r{r_q_a.inverse()};
  // Eigen::Quaterniond const a_q_r{a_q_b_ * b_q_r};
  Eigen::Matrix<double, kResidualSize, 1> error;
  error.head<3>() = Sophus::SO3d{a_q_r * r_q_a}.log();
  error.tail<3>() = a_q_r * (r_t_a - r_t_b) + a_t_b_;
  ...
  if (jacobians != nullptr) {
    ...
    if (jacobians[2] != nullptr) {
       ...
       *           = -Jr^{-1}(e_w)·r_R_a^{-1}
       *
       * d/dwb e_t = d/dwb a_R_b·(Exp(wb)·r_R_b)^{-1}·(r_t_a - r_t_b)
       *           = d/dwb a_R_b·r_R_b^{-1}·Exp(-wb)·(r_t_a - r_t_b)
       *           = d/dwb a_R_b·r_R_b^{-1}·(I - [wb]x)·(r_t_a - r_t_b)
       *           = d/dwb a_R_b·r_R_b^{-1}·[-wb]x·(r_t_a - r_t_b)
       *           = d/dwb a_R_b·r_R_b^{-1}·[r_t_a - r_t_b]x·wb
       *           = a_R_b·r_R_b^{-1}·[r_t_a - r_t_b]x
       */
      // clang-format on
      Eigen::Matrix<double, kResidualSize, 4, Eigen::RowMajor> de_dwb;
      de_dwb.setZero();
      ...
      de_dwb.block<3, 3>(3, 0) =
          (a_q_b_ * r_q_b.inverse()).toRotationMatrix() * Sophus::SO3d::hat(r_t_a - r_t_b);
